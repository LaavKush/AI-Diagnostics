...
import { alertText, chatBubbles } from '@/data/data'
...
const AlertSection = () => {
  return (
    <section className="min-h-[130vh] bg-[#fdfaf7] flex flex-col justify-center items-center py-32 px-6 md:px-20">

      <div className="max-w-[1600px] w-full grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
        
        {/* LEFT TEXT BLOCK */}
        <div className="flex flex-col justify-center">
          <h2 className="text-[2.8rem] md:text-[4rem] font-bold text-[#0d1b2a] mb-14 leading-tight">
            {alertText.heading}
          </h2>

          {alertText.items.map((item, i) => (
            <div key={i} className="mb-10 flex items-start gap-6">
              <div className="bg-[#fff4e5] text-[#0d1b2a] font-extrabold rounded-xl shadow px-5 py-3 text-2xl">
                {i + 1}
              </div>
              <div>
                <h3 className="text-3xl font-bold mb-2 text-[#0d1b2a]">{item.title}</h3>
        ...
        {/* RIGHT CHAT UI */}
<div className="relative bg-gradient-to-br from-[#eae6ff] to-[#fef6f4] p-10 rounded-[50px] shadow-2xl flex justify-center items-center">
